  ...
  Game.prototype = {
    ...
    onInput: function (e) {
      ...
      if (this.validateMatrix()) {
        const allFilled = Object.values(this.matrix.row).every((r) =>
          r.every((v) => v !== "")
        );
        if (allFilled) {
...
document.addEventListener(
  ...
  function (e) {
    ...
    var inputs = Array.from(
      document.querySelectorAll(".sudoku-container input")
    );
    inputs.forEach(function (inp) {
      inp.classList.toggle(
        "highlight",
        inp.value && el.value && inp.value === el.value
      );
    });
  },
  false
...
function gameWon() {
  if (currentDifficultyIndex === difficultyOrder.length - 1) {
    showToast(
      "🎉 Congratulation you are on highest level restarting game!",
      1500
    );
  } else {
